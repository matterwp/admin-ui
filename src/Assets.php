...
class Assets {
	 ...
	 * @return void
	 */
	public static function enqueueStyle( string $handle, string $url, string $path, array $dependencies = array(), string $fallback_version = '1.1.0' ): void {
		wp_enqueue_style(
			$handle,
			$url,
			$dependencies,
	 ...
	 * @return void
	 */
	public static function enqueueScript( string $handle, string $url, string $path, array $dependencies = array(), bool $in_footer = true, string $fallback_version = '1.1.0' ): void {
		wp_enqueue_script(
			$handle,
			$url,
			$dependencies,
	...
	public static function enqueueBuiltStyle( string $handle, array $dependencies = array(), string $version = '' ): void {
		...
		if ( '' === $version ) {
			$path    = $dir . '/' . $file;
			$version = file_exists( $path ) ? (string) filemtime( $path ) : '1.1.0';
		}
